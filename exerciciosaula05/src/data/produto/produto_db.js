...
let listarProdutos = () => {
    return realm_produto.objects('Produto1');
}
...
let adicionarProdutos = (nome, descricao, preco) => {
    ...
    const ultimoId = realm_produto.objects('Produto1').sorted('produto_id', true)[0];
    const maiorId = ultimoId == null ? 0 : ultimoId.produto_id;
    const proximoId = maiorId == null ? 1 : maiorId + 1;

    realm_produto.write(() => {
        const prod = realm_produto.create('Produto1', {
...
let deleteProduto = (id) => {
    console.log(id);
    realm_produto.write(() => {
        let deleteItem = realm_produto.objectForPrimaryKey('Produto1', id);
